...
- **Chia đều số bài crawl theo nguồn (2026-07-10):** thêm công tắc `EVEN_DISTRIBUTE_ACROSS_SOURCES` (mặc định tắt, giữ hành vi cũ) — khi bật, `_crawl_sources()` chia đều `MAX_ARTICLES_PER_JOB` cho từng nguồn đã chọn theo đúng thứ tự `source_ids` (dư dồn nguồn đầu). Giải quyết vấn đề đã ghi nhận nhiều lần ở Slice 2/3/4 verify: nguồn đầu tiên luôn "ăn hết" ngân sách, không test được đa dạng nguồn với số bài nhỏ. **Đã verify job thật** (job `2078ac9a-6697-4872-a54a-89b70cd9bc56`, 3 nguồn VTV/VOV/vietnam.vn, `MAX_ARTICLES_PER_JOB=5`): kết quả VTV=2, VOV=2, vietnam.vn=1, đúng phân bổ `_distribute_evenly(5, 3) = [2, 2, 1]`, `status=completed`, `.docx`/`.json` hợp lệ (magic bytes `PK\x03\x04`). 120/120 test pass (113 baseline + 7 mới).
- **Đổi sang water-filling — bù quota thiếu hụt giữa các nguồn (2026-07-10, revision 2):** qua test thật của user ngay sau khi merge (3 nguồn VTV/Vietnam.vn/TinGia cùng ngày, TinGia không có bài) phát hiện thiết kế "không bù thiếu hụt" ban đầu gây lãng phí ngân sách rõ rệt: TinGia không dùng hết quota 2 của nó, nhưng phần dư không chuyển được cho VTV/Vietnam.vn dù 2 nguồn này thừa khả năng lấy thêm (job 2 nguồn không có TinGia, cùng ngày, VTV lấy được 3 bài — chứng minh đủ dữ liệu, chỉ bị quota ban đầu giới hạn ở 1). User yêu cầu đổi sang có bù — sửa `_crawl_sources()`: quota mỗi nguồn tính LẠI ngay trước khi xử lý (`_distribute_evenly(ngân_sách_còn_lại, số_nguồn_chưa_xử_lý)[0]`) thay vì tính 1 lần cố định trước loop, tái dùng đúng `_distribute_evenly()` đã có (không cần hàm mới). Cập nhật spec (`docs/superpowers/specs/2026-07-10-even-distribute-sources-design.md`, revision 2), test `test_crawl_sources_does_not_compensate_shortfall_from_other_sources` → đổi thành `test_crawl_sources_compensates_shortfall_from_earlier_sources` (kỳ vọng tổng = `MAX_ARTICLES_PER_JOB` thay vì ít hơn). **Đã verify job thật** (job `57418b3c-6d0d-4298-a484-3913846d5213`, đúng lại kịch bản user báo lỗi — TinGia/Vietnam.vn/VTV, cùng ngày `2026-07-10`, `MAX_ARTICLES_PER_JOB=5`): kết quả TinGia=0, Vietnam.vn=3, VTV=2, **tổng đúng 5/5** (trước đó chỉ 3/5) — khớp chính xác toán water-filling. 120/120 test pass.
- **Sửa 2 hạn chế sitemap index (VOV tháng hiện tại + VTV mấy ngày gần nhất) — 2026-07-10:** user báo cáo trực tiếp qua quan sát thật (không phải verify job): (1) `vov.vn/sitemap.xml` chỉ thêm entry sub-sitemap của 1 tháng SAU KHI tháng đó kết thúc → job chạy giữa tháng 7 không bao giờ thấy sub-sitemap tháng 7 (chỉ tới tháng 6); (2) `vtv.vn` build sub-sitemap theo khối 5 ngày → vài ngày gần nhất (khối chưa đóng) không nằm trong sub-sitemap nào, không lấy được bài mới nhất. User tự phát hiện 2 URL "lối tắt" hợp lệ qua kiểm tra tay: `https://vov.vn/sitemaps/{year}/{month}/article.xml` (dự đoán được, verify curl trả HTTP 200 dù chưa lên index) và `https://vtv.vn/latest-news-sitemap.xml` (đã có sẵn trong chính index nhưng bị regex `_SITEMAP_DATE_PATTERNS` loại vì không mang ngày tháng trong path). Sửa `sitemap.py`: thêm `_SITEMAP_URL_TEMPLATES` (VOV — bỏ hẳn fetch/parse index, tự sinh URL sub-sitemap cho mọi tháng trong `date_from`–`date_to`, tái dùng `_fetch_declared_sitemap_pages()` đã có) và `_SITEMAP_ALWAYS_INCLUDE` (VTV — luôn fetch kèm URL catch-all cố định, không qua bộ lọc ngày). Xoá 2 test cũ test hành vi index-based của VOV (không còn đúng sau khi đổi cơ chế), thêm 4 test mới (3 cho VOV template, 1 cho VTV always-include) + sửa 3 test VTV hiện có (mock thêm request `latest-news-sitemap.xml`). **Đã verify job thật cả 2 nguồn:** VOV (`job b638f4a4...`, `date_from=2026-07-01`/`date_to=2026-07-10`) → 5/5 bài crawl được, toàn bộ thuộc tháng 7 thật (trước đây sẽ là 0 bài); VTV (`job 5131a68e...`, `date_from=2026-07-09`/`date_to=2026-07-10`) → 5/5 bài, `published_at` toàn bộ đúng `2026-07-10` (hôm nay), log xác nhận `latest-news-sitemap.xml` trả HTTP 200 và được fetch dù không khớp pattern ngày. 122/122 test pass (120 + 2 mới ròng sau khi trừ 2 test bỏ + thêm 4 test mới).
