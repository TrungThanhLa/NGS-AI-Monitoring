...
- **`_YEAR_MONTH_RE` ở `sitemap.py` (nhận diện sub-sitemap dạng `news-YYYY-M.xml`) có rủi ro false-positive lý thuyết** với tên sub-sitemap không theo ngày nhưng tình cờ khớp dạng `NNNN-N` (VD `category-2024-1.xml`) — sẽ tính sai khoảng ngày và có thể bỏ qua nhầm sub-sitemap đó thay vì rơi vào nhánh an toàn "không nhận diện được pattern → luôn fetch". Chấp nhận tạm vì cả 3 nguồn dùng pattern này (VOV/VietnamPlus/CAND) đều đã verify thật không gặp trường hợp này — cần xem lại nếu thêm nguồn mới dùng cùng pattern năm-tháng mà tên sub-sitemap có dạng số dễ nhầm (2026-06-30)
