...
- tingia.gov.vn: chuyển từ listing-page 1 trang sang crawl sitemap curated (2026-07-08) — xác nhận thật qua curl: `sitemap.xml` gốc là `<urlset>` phẳng (không phải `<sitemapindex>`) trộn 18 sub-sitemap chia theo CHỦ ĐỀ (không theo ngày) + 3 trang tĩnh, mọi `<lastmod>` đóng băng cùng timestamp build — không dùng được cơ chế index/`_SITEMAP_DATE_PATTERNS` hiện có. Thêm `parsing_rules.sitemap_pages` (danh sách 5/18 sub-sitemap do user chọn thủ công) + hàm `_fetch_declared_sitemap_pages()` trong `sitemap.py` (dedup URL cross-sub-sitemap vì 1 bài có thể gắn nhiều tag), 5 test mới, migration `0006` (`sitemap_url=NULL`, `listing_url=NULL`). **Đã verify job thật thành công** — xem "Verify Slice 2" bên dưới
- Thêm nguồn thứ 7 vietnam.vn (2026-07-08) — xác nhận thật qua curl: `sitemap.xml` LÀ `<sitemapindex>` thật (khác TinGia/BoCongAn), 1285 sub-sitemap `sitemap-post/YYYY-MM-DD.xml` chia đúng 1 ngày/file với `<lastmod>` thật riêng biệt (không đóng băng) + 16 sub-sitemap khác không phải bài viết (page/author/organization/category/tag×11/news). Dùng lại đúng cơ chế `_SITEMAP_DATE_PATTERNS` sẵn có (không cần curated list) — thêm convention thứ 3 "1 ngày" (`day` đơn lẻ) vào `_sub_sitemap_date_range()` + entry `vietnam.vn`, 3 test mới, migration `0007` seed nguồn. **Đã verify job thật thành công**
...
- **Slice 3: hoàn thành (2026-07-09).** `AI_CONCURRENCY`/`analyze_articles_batch()`/`ai_model` code xong + đã verify job thật 2 giai đoạn:
  ...
  - **Phát hiện thật cần theo dõi (chưa xử lý, không chặn Slice 3):** ở Giai đoạn A, 1/5 bài AI trả `emotion="Neutral"` — **không thuộc 6 nhóm cảm xúc chuẩn** đã định nghĩa (`Trust, Fear, Anger, Surprise, Sadness, Happy`, xem [07 · AI Pipeline](.claude/rules/07-ai-pipeline.md)). Code hiện không validate giá trị `emotion` trả về khớp enum trước khi lưu DB. Không xuất hiện lại ở mẫu 14 bài Giai đoạn B, nhưng cần theo dõi thêm khi có nhiều dữ liệu hơn — nếu lặp lại thường xuyên, cân nhắc thêm validate hoặc viết `v2.py` cho prompt
- Slice 4–6: chưa bắt đầu
...
| Listing-page crawler chỉ hỗ trợ 1 trang, không phân trang | YAGNI — nguồn cần đến lúc quyết định (tingia.gov.vn) không có phân trang thật; mở rộng khi có nguồn thật cần. **Cập nhật 2026-07-08:** tingia.gov.vn nay đã chuyển sang sitemap curated, cơ chế 1-trang này tạm không còn nguồn thật nào dùng — vẫn giữ lại làm fallback tổng quát |
...
| `celery-worker` cần `docker compose restart` (không cần rebuild) sau khi sửa code Python trong `backend/` | Volume mount `./backend:/app/backend` chỉ đồng bộ file, không tự nạp lại module đã import vào memory của process Celery đang chạy — sửa `sitemap.py` xong chạy job thật ngay lập tức vẫn dính lỗi `TypeError` của code CŨ, phải restart mới thấy code mới có hiệu lực (phát hiện thật khi verify job TinGia, 2026-07-08) |
| Sửa biến trong `.env` (VD `AI_CONCURRENCY`, `MAX_ARTICLES_PER_JOB`) phải dùng `docker compose up -d <service>` (recreate container) — **`restart` KHÔNG đủ**, khác hẳn trường hợp sửa code `.py` ở trên | `docker compose restart` chỉ khởi động lại process trong container cũ, giữ nguyên env lúc container được tạo — không đọc lại `.env`. Chỉ `up -d` mới khiến Compose phát hiện `.env` đổi và tạo lại container với env mới. Phát hiện thật khi làm Task 5 Slice 3 (2026-07-08): subagent báo cáo đã verify env var có trong container nhưng thực ra không có — tự tay kiểm tra lại mới phát hiện. `postgres` cũng dùng `env_file: .env` nên có thể bị recreate theo khi `up -d` — an toàn vì dùng named volume `pg_data`, không mất dữ liệu |
| vietnam.vn: thêm convention thứ 3 "1 ngày" (named group `day` đơn lẻ) vào `_sub_sitemap_date_range()`, tái dùng `_SITEMAP_DATE_PATTERNS` thay vì `sitemap_pages` curated | Xác nhận thật (curl 2026-07-08): sub-sitemap `sitemap-post/YYYY-MM-DD.xml` chia đúng theo NGÀY thật (khác TinGia chia theo chủ đề, `<lastmod>` không đóng băng) — hợp với cơ chế index/date-pattern sẵn có, chỉ thiếu convention "đúng 1 ngày/file" (2 convention cũ: `day_start+day_end` = khoảng ngày kiểu VTV, `year+month` = cả tháng kiểu VOV/VN+/CAND). 16 sub-sitemap không phải bài viết (page/author/organization/category/tag×11/news) tự động bị loại vì không khớp regex, không cần code exclude riêng |
| `get_article_urls()` check `status_code >= 400` của sitemap index (dùng lại `_fetch_with_retry`) trước khi parse, trả về index URL như 1 `failed_loc` nếu lỗi | Bug thật (vietnam.vn bị chặn 403 tạm thời, 2026-07-08): code cũ parse thẳng response lỗi, không tìm thấy `<sitemap>`/`<url>` nào → hiểu nhầm thành "sitemap phẳng 0 bài" → job báo `completed` thay vì lộ lỗi thật, không ai biết là do bị chặn. Áp dụng cho mọi nguồn dùng sitemap index (VTV/VOV/VietnamPlus/CAND/vietnam.vn) |
| Slice 3 Task 4: `_analyze_articles` gửi cả batch `pending` vào 1 lệnh `analyze_articles_batch()`, KHÔNG chia nhỏ theo chunk để commit từng phần | Code review phát hiện: chia chunk chỉ giữ được dữ liệu `article_analysis` không bị mất khi job chết giữa chừng, nhưng KHÔNG giải quyết được vấn đề gốc — report của lần chạy lại vẫn thiếu bài vì `url_hash` unique toàn cục làm job mới tự skip URL đã crawl từ job cũ (xem "Vấn đề cần làm rõ" bên dưới + [10 · Error Handling](.claude/rules/10-error-handling.md)). Vì chunk không giải quyết được vấn đề thật, quyết định không thêm độ phức tạp đó — chấp nhận đánh đổi tạm thời, ghi lại rõ ràng thay vì vá nửa vời |
