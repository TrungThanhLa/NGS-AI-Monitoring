...
- [docs/ROADMAP_CONTINUOUS_MONITORING.md](docs/ROADMAP_CONTINUOUS_MONITORING.md) — lộ trình Phase 0–9 để sửa/bổ sung code hiện tại cho khớp nghiệp vụ đúng, đặc tả đầy đủ ở rule [15](.claude/rules/15-auth-rbac.md)–[18](.claude/rules/18-alert-case-management.md). Phase 0–3 (chốt phạm vi, Auth & RBAC, Campaign & Master Data, Scheduler & Continuous Crawl) đã hoàn thành trên `main` — sẵn sàng bắt đầu Phase 4 (Content Repository & Review Workflow).
